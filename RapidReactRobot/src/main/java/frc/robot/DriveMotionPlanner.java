...
public class DriveMotionPlanner implements CSVWritable {
    ...
    protected Translation2d updatePurePursuit(Pose2d current_state) {
        double lookahead_time = Constants.PathFollowing.kPathLookaheadTime;
        ...
        while (actual_lookahead_distance < Constants.PathFollowing.kPathMinLookaheadDistance &&
        ...
        if (actual_lookahead_distance < Constants.PathFollowing.kPathMinLookaheadDistance) {
            lookahead_state = new TimedState<>(new Pose2dWithCurvature(lookahead_state.state()
                    .getPose().transformBy(Pose2d.fromTranslation(new Translation2d(
                            (mIsReversed ? -1.0 : 1.0) * (Constants.PathFollowing.kPathMinLookaheadDistance -
